...
public class JwtAuthenticationConverter implements Converter<Jwt, AbstractAuthenticationToken> {
    ...
    @Override
    public AbstractAuthenticationToken convert(@SuppressWarnings("NullableProblems") Jwt jwt) {
    ...
    private Collection<? extends GrantedAuthority> extractResourceRoles(Jwt jwt) {
        ...
        resource = (!(resourceAccess.get(resourceId) instanceof Map)) ? new HashMap<>() : (Map<String, Object>) resourceAccess.get(resourceId);
        ...
        return resourceRoles.stream().map(role -> new SimpleGrantedAuthority("ROLE_".concat(role))).collect(Collectors.toList());
    }
